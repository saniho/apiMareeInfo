...
## Release beta (ex: 2.1.8-beta1)

Même processus que la release normale, avec ces différences :

### Étape 1 — Bumper la version

**ATTENTION** : Home Assistant **rejette** les points dans la partie bêta du manifest.
- ❌ `2.1.8.beta.1` → invalide
- ❌ `2.1.8b1` → valide mais pas l'format voulu
- ✅ `2.1.8-beta1` → format correct (tiret OK, pas de point)

Dans ces 3 fichiers :
- `const.py` → `__VERSION__ = "2.1.8-beta1"`
- `manifest.json` → `"version": "2.1.8-beta1"`
- `README.md` → idem
...
```bash
git add -A && git commit -m "chore: bump version to 2.1.8-beta1"
git push
```
...
curl -s -X POST \
  ...
  -H "Accept: application/vnd.github+json" \
  https://api.github.com/repos/saniho/apiMareeInfo/releases \
  -d '{"tag_name":"2.1.8-beta1","target_commitish":"feature/xxx","name":"v2.1.8-beta1","body":"<notes>","draft":false,"prerelease":true}'
```
